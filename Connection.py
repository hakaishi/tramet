...
class Connection:
    ...
    def connect(self, ui, mode, host, port, name, password, encoding, path, callback=None):
        if self._worker:
            self._worker.quit()
        self._worker = ThreadWork(mode, host, port, name, password, encoding)
        if self._ui_worker:
            self._ui_worker.quit()
        self._ui_worker = ThreadWork(mode, host, port, name, password, encoding)
        self._mode = mode
        self._enc = encoding
    ...
    def get_listing(self, ui, path, callback):
        def worker(conn, ui_, path_, enc, cb):
            result = []
            if self._mode == "SFTP":
                try:
                    if not path_:
        ...
        self._ui_worker.add_task(worker, [ui, path, self._enc, callback])

    def cwd_dnl(self, ui, path, item_info, update, done):
        def worker(conn, ui_, path_, enc, item_nfo, updatefunc, donefunc):
            if self._mode == "SFTP":
                inf = None
                if not path_:
                    channel = conn.session.open_session()
        ...
        self._ui_worker.add_task(worker, args=[
            ui, path, self._enc, item_info, update, done
        ])

    def search(self, path, recursive, depth, filename, sensitive, regex, insert_result, done):
        self.stop_search = False

        def worker(conn, path_, recursive_, depth_, filename_, sensitive_, regex_, resultfunc, donefunc):
            if self._mode == "SFTP":
                def recurse(pth):
                    ...
                    try:
                        ...
                        for p, i in data.items():
                            ...
                            if d[0][0] == "d":
                                ...
                                if not regex_ and ((sensitive_ and filename_ in p) or (not sensitive_ and filename_.lower() in p.lower())):
        ...
        self._ui_worker.add_task(worker, args=[path, recursive, depth, filename, sensitive, regex, insert_result, done])
    ...
    def download(self, ui, src, file, ts, update, done, isFile=True, destination=""):
                    ...
                    if overwrite:
                        if self._mode == "SFTP":
                            ...
                            for inf in dat.items():
                                recrse(join(src_, file_, inf[0]), inf[1], size_all)
        ...
        self._worker.add_task(worker, args=[ui, src, file, ts, update, done, isFile, destination])

    def download_multi(self, ui, selection, update, done):
        def worker(conn, ui_, sel, updatefunc, donefunc):
            destination = filedialog.askdirectory(
                title="Choose download destination")
            if not destination:
                return
            ...
            for item in sel:
                ...
                if self._mode == "SFTP":
                    ...
                    self._worker.add_task(
                        self.download,
                        args=[
                            ui_, self.cwd, item["text"],
                            (ts, ts), None, donefunc, isFile, destination
                        ]
                    )

        self._ui_worker.add_task(worker, [ui, selection, update, done])

    def upload_files(self, ui, files, dest, update, done):
        def worker(conn, files_, destination, updatefunc, donefunc):
        ...
        self._worker.add_task(worker, args=[files, dest, update, done])

    def upload_folder(self, ui, folder, destination, update, done):
        def worker(conn, ui_, folder_, destination_, updatefunc, donefunc):
            size_all = {"size": 0}
        ...
        self._worker.add_task(worker, args=[ui, folder, destination, update, done])

    def mkdir(self, ui, name, callback):
        def worker(conn, name_, cb):
            if self._mode == "SFTP":
                try:
                    flgs = LIBSSH2_FXF_CREAT | LIBSSH2_SFTP_S_IRWXU | \
                           LIBSSH2_SFTP_S_IRWXG | LIBSSH2_SFTP_S_IXOTH | \
        ...
        self._worker.add_task(worker, args=[name, callback])

    def rename(self, ui, orig_name, new_name, callback):
        def worker(conn, orig, new_, cb):
            if self._mode == "SFTP":
                try:
                    conn.rename(
                        join(self.cwd, orig),
        ...
        self._worker.add_task(worker, args=[orig_name, new_name, callback])

    def delete_object(self, ui, list_, callback):
        def worker(connection, list__, callback_):
        ...
        self._worker.add_task(worker, args=[list_, callback])
